...
formulas <- list(
  ...
  update(baseForm, bin_married ~ . + l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married),
...
direct_effect_rules <- list(
  ...
  ## Censoring 
  censor = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 9)
)
...
## Aggregate
all_runs <- rbindlist(lapply(c('natural_black','natural_white','intervention','direct'), compile_sims_simple, bs = bootruns))
...
cov_obs <- actualDF %>%
  ...
  filter(measure!='censor') %>%
  mutate(measure=factor(measure, levels = c('mppvt','mwodtke', unique(actualDF$measure[!(actualDF$measure %in% c('mppvt','mwodtke'))]))))
ggplot(data=cov_plot %>% filter((measure %in% c('pov','married','jail'))), aes(x=age, y=mean)) +
  geom_line(aes(color=type), size=1) +
  geom_ribbon(aes(ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  geom_point(data=cov_obs %>% filter((measure %in% c('pov','married','jail'))), aes(fill=type), shape=21, color='black', size=5) + 
  theme_classic() +

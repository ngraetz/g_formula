...
## Lag it all.
tc_vars <- c('f_age', 'f_immigrant', 'm_age', 'm_living_parents_15', 'm_immigrant', 'm_cognitive', 'm_race')
tc_vars_child <- c('c_sex_male', 'c_lbw')
tv_vars <- c('c_ppvt','bin_unemployed', 'bin_lesshs', 'bin_pov', 'm_f_in_jail', 'm_depression', 'bin_married','wodtke')
DF <- as.data.table(DF)
...
## PLOTS
DF %>% filter(m_race %in% c('white','black')) %>% group_by(m_race, id) %>% summarise(wodtke=mean(d.wodtke)) %>% ggplot(aes(x=wodtke, fill=m_race)) +
...
# formula for models
formulas <- list(
  update(baseForm, c_ppvt ~ . + age * (d.wodtke + d.bin_pov + l.bin_lesshs + l.bin_unemployed + d.m_f_in_jail + l.m_depression + l.bin_married) + c_sex_male + c_lbw),
  update(baseForm, bin_married ~ . + age * (l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married)),
  update(baseForm, bin_lesshs ~ . + l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married),
  update(baseForm, bin_pov ~ . + age * (l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married)),
  update(baseForm, m_f_in_jail ~ . + l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married),
  update(baseForm, m_depression ~ . + d.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married),
  update(baseForm, bin_unemployed ~ . + age * (l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married)),
  update(baseForm, wodtke ~ . + l.wodtke + l.bin_pov + l.bin_lesshs + l.bin_unemployed + l.m_f_in_jail + l.m_depression + l.bin_married),
  as.formula(paste0('censor ~ ', paste(tc_vars, collapse = ' + '), ' + 
                    (age) * (', paste(tv_vars, collapse = ' + '), ')')) 
  )
families <- list(gaussian, binomial, binomial, binomial, binomial, binomial, binomial, gaussian, binomial) # families for models
functions <- list(glm, glm, glm, glm, glm, glm, glm, glm, glm) # functions for results
...
lags <- list(
  ...
  l.bin_married = function(DF) DF %>% mutate(l.bin_married=lag(bin_married)),
  l.bin_lesshs = function(DF) DF %>% mutate(l.bin_lesshs=lag(bin_lesshs)),
...
natural_rules <- list(
  ...
  bin_married = function(DF, models, ...) simPredict(DF, models, 2),
  bin_lesshs = function(DF, models, ...) simPredict(DF, models, 3),
  bin_pov = function(DF, models, ...) simPredict(DF, models, 4),
  m_f_in_jail = function(DF, models, ...) simPredict(DF, models, 5),
  m_depression = function(DF, models, ...) simPredict(DF, models, 6),
  bin_unemployed = function(DF, models, ...) simPredict(DF, models, 7),
  wodtke = function(DF, models, ...) simPredict(DF, models, 8),
  censor = function(DF, models, ...) simPredict(DF, models, 9)
)
...
intervention_rules <- list(
  ...
  wodtke = function(DF, ...) DF %>% mutate(wodtke = -3) %>% select(wodtke)
)
...
direct_effect_rules <- list(
  ...
  bin_pov = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 4),
  m_f_in_jail = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 5),
  m_depression = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 6),
  bin_unemployed = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 7),
  bin_married = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 2),
  ## Direct pathway
  #bin_married = function(DF, ...) DF %>% mutate(bin_married = replace(bin_married, bin_married==0, 1)) %>% select(bin_married),
  wodtke = function(DF, models, natural_DF, intervention_DF, ...) simScenario(DF, models, intervention_DF, 8),
  #bin_married = function(DF, models, natural_DF, intervention_DF, ...) simScenario(DF, models, intervention_DF, 2),
  ## Censoring 
  censor = function(DF, models, ...) simPredict(DF, models, 9)
)

indirect_effect_rules <- list(
  ## DV of interest
  c_ppvt = function(DF, models, ...) simPredict(DF, models, 1),
  ## Indirect pathways
  bin_lesshs = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 3),
  bin_pov = function(DF, models, natural_DF, intervention_DF, ...) simScenario(DF, models, intervention_DF, 4),
  m_f_in_jail = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 5),
  m_depression = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 6),
  bin_unemployed = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 7),
  wodtke = function(DF, models, natural_DF, ...) simScenario(DF, models, natural_DF, 8),
  ...
  #bin_married = function(DF, ...) DF %>% mutate(bin_married = replace(bin_married, bin_married==0, 1)) %>% select(bin_married),
  #wodtke = function(DF, models, natural_DF, intervention_DF, ...) simScenario(DF, models, intervention_DF, 8),
  bin_married = function(DF, models, natural_DF, intervention_DF, ...) simScenario(DF, models, natural_DF, 2),
  ## Censoring 
  censor = function(DF, models, ...) simPredict(DF, models, 9)
)
...
bootruns <- lapply(1:boots, function(b) {
  ...
  # Simulate direct effect drawing stochastically from either the natural or intervention course according to the direct rules 
  direct_effect_DF_black <- progressSimulation(mcDF_all, lags, direct_effect_rules, gfitboot_all, natural_DF=natural_course_DF_all, intervention_DF=intervention_DF_black)
  
  # Simulate indirect effects [not tested]
  indirect_effect_DF_black <- progressSimulation(mcDF_all, lags, indirect_effect_rules, gfitboot_all, natural_DF=natural_course_DF_all, intervention_DF=intervention_DF_black)
  
  # Return all courses simulated
  list(natural=natural_course_DF_all,
       intervention=intervention_DF_black,
       direct=direct_effect_DF_black,
       indirect=indirect_effect_DF_black)
...
all_effects <- lapply(c('natural','intervention','direct','indirect'), compile_sims_table, bs=bootruns)
all_effects <- Reduce(merge, all_effects)
for(c in c('mppvt','married','lesshs','pov','jail','depression','unemployed','censor','mwodtke')) {
  for(b in 1:boots) all_effects[, (paste0(c, '_total_', b)) := get(paste0(c, '_intervention_', b)) - get(paste0(c, '_natural_', b))]
  for(b in 1:boots) all_effects[, (paste0(c, '_direct_', b)) := get(paste0(c, '_direct_', b)) - get(paste0(c, '_natural_', b))]
  for(b in 1:boots) all_effects[, (paste0(c, '_indirect_', b)) := get(paste0(c, '_indirect_', b)) - get(paste0(c, '_natural_', b))]
}
all_effects[, total_lower := apply(.SD, 1, quantile, c(.025), na.rm=T), .SDcols=grep('^mppvt_total_', names(all_effects))]
all_effects[, total_mean := apply(.SD, 1, mean), .SDcols=grep('^mppvt_total_', names(all_effects))]
all_effects[, total_upper := apply(.SD, 1, quantile, c(.975), na.rm=T), .SDcols=grep('^mppvt_total_', names(all_effects))]

all_effects[, direct_lower := apply(.SD, 1, quantile, c(.025), na.rm=T), .SDcols=grep('^mppvt_direct_', names(all_effects))]
all_effects[, direct_mean := apply(.SD, 1, mean), .SDcols=grep('^mppvt_direct_', names(all_effects))]
all_effects[, direct_upper := apply(.SD, 1, quantile, c(.975), na.rm=T), .SDcols=grep('^mppvt_direct_', names(all_effects))]

all_effects[, indirect_lower := apply(.SD, 1, quantile, c(.025), na.rm=T), .SDcols=grep('^mppvt_indirect_', names(all_effects))]
all_effects[, indirect_mean := apply(.SD, 1, mean), .SDcols=grep('^mppvt_indirect_', names(all_effects))]
all_effects[, indirect_upper := apply(.SD, 1, quantile, c(.975), na.rm=T), .SDcols=grep('^mppvt_indirect_', names(all_effects))]

all_effects[, total := paste0(round(total_mean,2), ' (', round(total_lower,2), ' - ', round(total_upper,2), ')')]
all_effects[, direct := paste0(round(direct_mean,2), ' (', round(direct_lower,2), ' - ', round(direct_upper,2), ')')]
all_effects[, indirect := paste0(round(indirect_mean,2), ' (', round(indirect_lower,2), ' - ', round(indirect_upper,2), ')')]
all_effects <- all_effects[, c('age','total','direct','indirect')]
all_effects <- redo_ages(all_effects)

## Make coefficient table with full DF
gfitboot_all <- gfit.init(formulas, families, functions, data=DF)

## Aggregate
all_runs <- rbindlist(lapply(c('natural','intervention','direct','indirect'), compile_sims_simple, bs = bootruns))
...
## Plot 
cov_plot <- all_runs %>%
  filter((measure=='mppvt')|(measure!='mppvt')) %>%
  ...
  mutate(type=factor(type,levels=c('natural','intervention','direct','indirect')))
cov_obs <- actualDF %>%
  mutate(type='observed') %>%
  filter((measure=='mppvt')|(measure!='mppvt')) %>%
  filter(measure!='censor') %>%
  mutate(measure=factor(measure, levels = c('mppvt','mwodtke', unique(actualDF$measure[!(actualDF$measure %in% c('mppvt','mwodtke'))]))))
ggplot(data=cov_plot %>% filter((measure %in% c('pov','jail') & type %in% c('natural'))), aes(x=age, y=mean)) +
  geom_line(aes(color=type), size=1) +
  geom_ribbon(aes(ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  #geom_point(data=cov_obs %>% filter((measure %in% c('pov','jail'))), aes(fill=type), shape=21, color='black', size=5) + 
...
ggplot(data=cov_plot %>% filter((measure %in% c('pov','jail') & type %in% c('natural','intervention'))), aes(x=age, y=mean)) +
  geom_line(aes(color=type), size=1) +
  geom_ribbon(aes(ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  #geom_point(data=cov_obs %>% filter((measure %in% c('pov','jail'))), aes(fill=type), shape=21, color='black', size=5) + 
  scale_fill_manual(values=c('#7fc97f','#beaed4','red','#ffff99')) + 
  ...
  facet_wrap(~measure, scales = 'free_y') +
  guides(fill=guide_legend(title='Simulated\ncourse'), color=FALSE) + labs(x='Child age',y='Population average (proportion or mean)')
ggplot(data=cov_plot %>% filter((measure %in% c('mppvt') & type=='natural')) %>% filter(age>=3), aes(x=age, y=mean)) +
  geom_line(aes(color=type), size=1) +
  geom_ribbon(aes(ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  #geom_point(data=cov_obs %>% filter((measure %in% c('mppvt'))) %>% filter(age>=3), aes(fill=type), shape=21, color='black', size=5) + 
  scale_fill_manual(values=c('#7fc97f','#beaed4','#fdc086','#ffff99')) + 
  ylim(c(85,100)) + 
  theme_classic() +
  facet_wrap(~measure, scales = 'free_y') +
  guides(fill=guide_legend(title='Simulated\ncourse'), color=FALSE) + labs(x='Child age',y='Population average (proportion or mean)')
ggplot(data=cov_plot %>% filter((measure %in% c('mppvt') & type %in% c('natural','intervention'))) %>% filter(age>=3), aes(x=age, y=mean)) +
  geom_line(aes(color=type), size=1) +
  geom_ribbon(aes(ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  #geom_point(data=cov_obs %>% filter((measure %in% c('mppvt'))) %>% filter(age>=3), aes(fill=type), shape=21, color='black', size=5) + 
  scale_fill_manual(values=c('#7fc97f','#beaed4','#fdc086','#ffff99')) + 
  ...
  guides(fill=guide_legend(title='Simulated\ncourse'), color=FALSE) + labs(x='Child age',y='Population average (proportion or mean)')
ggplot() +
  geom_ribbon(data=cov_plot %>% filter((measure %in% c('mppvt') & type %in% c('natural','intervention'))) %>% filter(age>=3),
              aes(x=age, ymin=lwr, ymax=upr, fill=type), alpha=.5) +
  geom_line(data=cov_plot %>% filter((measure %in% c('mppvt') & type %in% c('natural','intervention'))) %>% filter(age>=3),
            aes(x=age, y=mean, color=type), size=1) +
  geom_line(data=cov_plot %>% filter((measure %in% c('mppvt') & type %in% c('direct'))) %>% filter(age>=3),
            aes(x=age, y=mean), size=1, color='red') +  #geom_point(data=cov_obs %>% filter((measure %in% c('mppvt'))) %>% filter(age>=3), aes(fill=type), shape=21, color='black', size=5) + 
  scale_fill_manual(values=c('#7fc97f','#beaed4','red','#ffff99')) + 
  scale_color_manual(values=c('#7fc97f','#beaed4','red','#ffff99')) + 
  ylim(c(85,100)) + 
  theme_classic() +
  facet_wrap(~measure, scales = 'free_y') +
  guides(fill=guide_legend(title='Simulated\ncourse'), color=FALSE) + labs(x='Child age',y='Population average (proportion or mean)')
